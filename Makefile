...
c:
	gcc -o libethercatinterface.so -Wall -g -shared -fPIC ethercatinterface.c -I/opt/etherlab/include -L/opt/etherlab/lib -lethercat

## exec: execute the compiled main application
exec:
	sudo env LD_LIBRARY_PATH=$(PWD):/opt/etherlab/lib GOGC=500 chrt -f 80 ./jamun
...
# cp ./www_v2/*.* "$(dir_path)/www_v2"
	...
	du -sh "$(dir_path)"
# ðŸ”¹ THE FIX: Change into the release directory before zipping
# This prevents the "nested folder" bug that breaks the Go unzip logic
	cd ./release && tar -cvzf "$(fileName).tar.gz" "$(fileName)"
	rm -rf "$(dir_path)"
