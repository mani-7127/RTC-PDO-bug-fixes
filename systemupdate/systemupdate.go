...
func backupExistingSystem() (backupPath string, err error) {
	...
	// CHANGE THIS: Use a fixed path outside the working directory
	backupPathTmp := "/home/pi/jamun_backup" 
	
	// Ensure the base backup directory exists
	os.MkdirAll(backupPathTmp, 0777)

	backupPath = fmt.Sprintf("%s/%s", backupPathTmp, currentTime.Format("2006-01-02-15-04-05"))
	
	// Copy current working directory to the backup location
...
// SendBootSuccessNotification sends a signal to Azure/UI confirming 
// that the system has successfully booted and the drivers are OP.
func SendBootSuccessNotification() {
	// This ensures the "boot-hang" fix: we only notify success 
	// once the main logic confirms the drivers are working.
	logger.Info("System boot successful. Notifying update status...")
	
	// If you have a specific socket event or Azure update logic, call it here.
	// For example:
	// channels.SendAlarm("System Ready: Update Applied Successfully")
}
